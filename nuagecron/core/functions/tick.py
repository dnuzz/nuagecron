    ...
    while ready_schedules:
        for schedule in ready_schedules:
            ...
            if concurrency_action == ConcurrencyAction.skip:
                ...
                LOG.info(f'Schedule {schedule.project_stack}/{schedule.name} is set to be skipped so it will be retried on {schedule.next_run}')
            elif concurrency_action == ConcurrencyAction.block:
                LOG.info(f'Schedule {schedule.project_stack}/{schedule.name} is set to be blocked so will be retried on next run of tick')
                ...
                LOG.info(f'Schedule {schedule.project_stack}/{schedule.name} was invoked with execution time {schedule.next_run}')
